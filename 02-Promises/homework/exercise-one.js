...
function problemC () {
  ...
  // ???
  Promise.all([promisifiedReadFile('poem-one/stanza-02.txt'),promisifiedReadFile('poem-one/stanza-03.txt')]).then((all)=>{
    all.forEach((elemento) => blue(elemento))
    console.log(Done)})
...
function problemE () {
  ...
  // promise version
  // ???
  // Promise.all([promisifiedReadFile('poem-one/stanza-03.txt'),promisifiedReadFile('poem-one/wrong-file-name.txt')]).then((all)=>{
  //   all.forEach((elemnto)=>{
  //     (elemnto)=>{},
  //     ()=>{}
  //   })
